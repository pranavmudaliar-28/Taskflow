...
const statusConfig: Record<string, { label: string; cls: string; dot: string }> = {
  todo: { label: "To Do", cls: "bg-muted text-muted-foreground border-border", dot: "bg-muted-foreground/30" },
  in_progress: { label: "In Progress", cls: "bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20", dot: "bg-blue-500" },
  in_review: { label: "In Review", cls: "bg-violet-500/10 text-violet-600 dark:text-violet-400 border-violet-500/20", dot: "bg-violet-500" },
  testing: { label: "Testing", cls: "bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-100/20", dot: "bg-amber-500" },
  done: { label: "Done", cls: "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-100/20", dot: "bg-emerald-500" },
};

const priorityConfig: Record<string, { cls: string }> = {
  low: { cls: "bg-muted text-muted-foreground border-border" },
  medium: { cls: "bg-blue-500/10 text-blue-500 dark:text-blue-400 border-blue-500/20" },
  high: { cls: "bg-orange-500/10 text-orange-600 dark:text-orange-400 border-orange-500/20" },
  urgent: { cls: "bg-red-500/10 text-red-600 dark:text-red-400 border-red-500/20" },
};
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div className="divide-y divide-border/30">
              {tasksLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-sm font-medium text-muted-foreground">No recent tasks</p>
                  <p className="text-xs text-muted-foreground/60 mt-1">Tasks will appear here as you create them</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-5">
            ...
            <div className="bg-card rounded-xl border border-border shadow-sm">
              <div className="flex items-center justify-between px-5 py-4 border-b border-border/50">
                <h2 className="text-sm font-semibold text-foreground">Recent Time Logs</h2>
              ...
              <div className="px-5 py-3 space-y-2">
                {timeLogsLoading ? (
                  ...
                  <p className="text-xs text-muted-foreground text-center py-4">No time logs yet</p>
                )}
              </div>

              {stats?.totalTimeLogged != null && stats.totalTimeLogged > 0 && (
                <div className="mx-5 mb-4 rounded-lg bg-blue-500/10 border border-blue-500/20 px-4 py-3 flex items-center gap-3">
                  <TrendingUp className="h-4 w-4 text-blue-500 shrink-0" />
                  <div>
                    <p className="text-xs font-semibold text-blue-700 dark:text-blue-400">Total logged</p>
                    <p className="text-sm font-bold text-blue-900 dark:text-blue-100">{formatDuration(stats.totalTimeLogged)}</p>
                  </div>
                </div>
              )}
            </div>
